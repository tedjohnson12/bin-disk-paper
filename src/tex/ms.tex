...
\begin{figure*}
    \begin{centering}
        ...
        \caption{
            Same as Figure \ref{fig:ebin}, but with the mutual inclination distribution given by equation (\ref{eq:idist}). \RR{  The insert shows the mutual inclination probability distribution normalized by $\sin i$. This normalization shows that the probability-per-unit-area on the unit sphere becomes very high at $i=0$ (coplanar alignment) when $\sigma$ is small (dashed and dotted lines), but it is uniform everywhere in the isotropic case ($\sigma \rightarrow \infty$, solid line).   For low $j$, lower inclination preferences decrease $f_\text{polar}$ around $e_\text{b}\sim0.5$, but have little effect near $e_{\rm b}=0$ and 1. For high-$j$, low inclinations lower the value that $f_\text{polar}$ tends to as $e_\text{b}\rightarrow 0$.}
        }
...
\RR{
    When disk breaking and gravitational instability are considered jointly it may be that $f_\text{polar}$, as calculated in this study, is not as relevant as ``the fraction of binary systems which host a polar circumbinary disk.'' Gravitational instability (if not suppressed by heating as mentioned previously) requires that massive high-$j$ disks be more radially extended, making them more susceptible to differential precession and breaking. The result is that a circumbinary system initially hosting a single, extended, misaligned disk may evolve to host a series narrower rings, with the innermost most likely to have completed its evolution to a coplanar or polar alignment.
}
...
\RR{
A similar method has been proposed by \citet{zhang2019} to distinguish polar from coplanar planets in eclipsing binary systems via eclipse timing variations. I contrast to the imaging method suggested previously, such a method would be less expensive, have fewer degeneracies, and have many more potential targets as it would not be restricted to young systems. Additionally, it would be most sensitive to planets with small separations from their host binaries, meaning the alignment timescale for the progenitor disk would be much shorter than for those that could be resolved via imaging. Together, these two methods could provide robust constraints of $f_\text{polar}$ across many regimes of host parameters and disk separation.
}
