...
\begin{equation}
    t_\text{p} = \frac{16}{3f(1-f)} \Omega_\text{b}^{-1} \left(\frac{r}{a_\text{b}}\right)^{7/2} g(e_\text{b},i,\Omega)\, ,
\end{equation}
where $\Omega_\text{b}$ is the binary orbital frequency and $g$ is a dimensionless constant that depends on the initial conditions:
\begin{equation}
    g(e_\text{b},i,\Omega) = \frac{K(k^2)}{\sqrt{(1-e_\text{b}^2)(h+4e_\text{b}^2)}} \, .
\end{equation}
...
Note that $g$ depends on $e_\text{b}$, and so it is only a constant in the case of a test particle. For $j>0$, we must compute the precession timescale ourselves. The time coordinate used in Section \ref{sec:rk} is $\tau \equiv \alpha' t$. $\alpha'$ is defined by \citet[see eq. 3.9]{farago2010}, and we write it as
\begin{equation}
    \alpha' \equiv \frac{3}{4} \Omega_\text{b} \left(\frac{a_\text{b}}{r}\right)^{7/2} f(1-f) \sqrt{1+f}\, .
\end{equation}

Given some precession timescale in code units $\tau_\text{p}$, we can calculate the precession timescale
\begin{equation}
    t_\text{p} = \frac{4}{3}\Omega_\text{b}^{-1} \left(\frac{r}{a_\text{b}}\right)^{7/2} \frac{\tau_\text{p}}{f(1-f)\sqrt{1+f}}
\end{equation}




% \citet{rabago2024} gives the timescale for nodal precession of a thin ring to be
% \begin{equation}
%     t_\text{p} = \frac{1}{k} \left( \frac{r}{a_\text{b}} \right)^{7/2} t_\text{b}
% \end{equation}
% where $t_\text{b}$ is the binary orbital timescale, $r$ is the radius of the ring, and $k$ is a scaling factor that depends on the mass fraction and binary eccentricity. $|k| < 1$ \citep[see their equation (11), also][]{lubow2018,smallwood2019}, and, for a circumbinary disk, $r > a_\text{b}$. 
