...
\newcommand{\RP}[1]{\RR{#1}} % Retrograde/Prograde confusion
...
% \RP{Retrograde nodal precession takes over from prograde nodal precession near the anti-aligned point}, and {\it crescent} orbits take over at inclinations between libration and high-inclination precession (\citealt{chen2019}, see also \citealt{zanazzi2018}), named because of the shape they form on a phase diagram.
...
\begin{equation}
    ...
    \label{eq:omega}
\end{equation}
% If a disk undergoes \RP{retrograde} nodal precession, then \RR{the path $(x,\,y)$} forms clockwise loops about the origin as the system evolves. Under \RP{prograde} precession, these loops are counter-clockwise.
...
\RR{
    In this work we have only considered disks orbiting isolated binary systems, however, we can qualitatively extend our results to hierarchical triple star systems as well. There are two such cases to consider: 1) the outer-companion case where third star orbits exterior to the disk, and 2) the inner-companion case, where the disk orbits the tripe-star system. In both the outer \citep[see][]{martin2022,ceppi2023} and inner \citep[see][]{lepp2023,lepp2025} cases, additional conditions are imposed for libration to occur, and $f_\text{polar}$ decreases. The magnitude of these effects is dependent on the geometry of the triple-star system.
}
